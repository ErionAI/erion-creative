...
export function MediaModal({
  ...
  const { focusedItemIndex, setFocusedItemIndex } = useCreativeStore();

  const handleDownload = useCallback(() => {
    const url = resultVideo || resultImages[focusedItemIndex || 0];
    if (!url) return;
  ...
  }, [resultVideo, resultImages, focusedItemIndex, type]);
  ...
  if (focusedItemIndex === null || (!resultImages[focusedItemIndex] && !resultVideo)) {
    return null;
  }
